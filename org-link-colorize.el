...
(defcustom org-link-colorize-video-preview (or (executable-find "ffmpegthumbnailer")
...
(defcustom org-link-colorize-thumbnails-dir 'source-path
...
(defcustom org-link-colorize-video-preview-size 512
  "The video thumbnail image size."
  :type 'number
  :safe #'numberp
  :group 'org-link-colorize)

(defcustom org-link-colorize-video-preview-list
...
(defcustom org-link-colorize-audio-preview (or (executable-find "audiowaveform")
                                               (executable-find "qlmanage"))
...
(defcustom org-link-colorize-audio-preview-list '("mp3" "wav" "flac" "ogg" "dat")
  "A list of audio file types be supported generating audio wave form image."
  :type 'list
  :safe #'listp
  :group 'org-link-colorize)

(defcustom org-link-colorize-audio-preview-size 150
  "The audio wave form image size."
  :type 'number
  :safe #'numberp
  :group 'org-link-colorize)

(defcustom org-link-colorize-pdf-preview (or (executable-find "pdftocairo")
...
(defcustom org-link-colorize-pdf-preview-command 'pdftocairo
...
(defcustom org-link-colorize-pdf-preview-size 512
  "The PDF preview image size."
  :type 'number
  :safe #'numberp
  :group 'org-link-colorize)

(defcustom org-link-colorize-pdf-preview-default-page-number 1
  "The default PDF preview page number."
  :type 'number
  :safe #'numberp
  :group 'org-link-colorize)

(defcustom org-link-colorize-pdf-preview-image-format 'png
...
(defcustom org-link-colorize-epub-preview
  (cl-case system-type
...
(defcustom org-link-colorize-epub-preview-size nil
  "The EPUB cover preview image size."
  :type 'number
  :safe #'numberp
  :group 'org-link-colorize)

(defcustom org-link-colorize-text-preview nil
  "Whether enable text files content preview?"
  :type 'boolean
  :safe #'booleanp
  :group 'org-link-colorize)

(defcustom org-link-colorize-text-preview-list
  '("org" "txt" "markdown" "md"
    "lisp" "scm" "clj" "cljs"
    "py" "rb" "pl"
    "c" "cpp" "h" "hpp" "cs" "java"
...
(defcustom org-link-colorize-enable-debug-p nil
  "Whether enable org-link-colorize print debug info."
  :type 'boolean
  :safe #'booleanp)
...
(defun org-link-colorize--ensure-thumbnails-dir (thumbnails-dir)
  "Ensure THUMBNAILS-DIR exist, if not ,create it."
  (unless (file-directory-p thumbnails-dir)
    (make-directory thumbnails-dir)))

(defun org-link-colorize--display-thumbnail (thumbnail thumbnail-size start end)
...
(defun org-link-colorize--preview-pdf (path start end)
  "Preview PDF file PATH and display on link between START and END."
  (if (string-match "\\(.*?\\)\\(?:::\\(.*\\)\\)?\\'" path)
      (let* ((file-path (match-string 1 path))
             ;; DEBUG: (_ (lambda (message "--> HERE org-link-colorize (pdf): path: %s" path)))
             (pdf-page-number (if (match-string 2 path)
                                  (string-to-number (match-string 2 path))
                                org-link-colorize-pdf-preview-default-page-number))
             (pdf-file (expand-file-name (org-link-unescape file-path)))
             (thumbnails-dir (pcase org-link-colorize-thumbnails-dir
                               ('source-path
                         ...
                         (concat
                          (if (= pdf-page-number 1)
                              (format "%s%s.%s"
                                      ...
                                      (symbol-name org-link-colorize-pdf-preview-image-format))
                            (format "%s%s-P%s.%s"
                                    thumbnails-dir (file-name-base pdf-file) pdf-page-number
                                    (symbol-name org-link-colorize-pdf-preview-image-format))))))
             (thumbnail-size (or org-link-colorize-pdf-preview-size 512)))
        (org-link-colorize--ensure-thumbnails-dir thumbnails-dir)
        (unless (file-exists-p thumbnail)
          (pcase org-link-colorize-pdf-preview-command
            ('pdftocairo
             ;; DEBUG:
             ;; (message
             ;;  "org-link-colorize: page-number %s, pdf-file %s, thumbnail %s"
             ;;  pdf-page-number pdf-file thumbnail)
             (start-process
              "org-link-colorize--pdf-preview"
              " *org-link-colorize pdf-preview*"
              "pdftocairo"
              (pcase org-link-colorize-pdf-preview-image-format
                ('png "-png")
                ('jpeg "-jpeg")
                ('svg "-svg"))
              "-singlefile"
             ...
             (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
               (message "[org-link-colorize] PDF create thumbnail for\n %s \nfailed." thumbnail)))
            ('pdf2svg
             (unless (eq org-link-colorize-pdf-preview-image-format 'svg)
               (warn "The pdf2svg only supports convert PDF to SVG format.
Please adjust `org-link-colorize-pdf-preview-command' to `pdftocairo' or
Set `org-link-colorize-pdf-preview-image-format' to `svg'."))
        ...
        (org-link-colorize--add-keymap start end)
        ;; display thumbnail only when it exist.
        (when (file-exists-p thumbnail)
          (org-link-colorize--display-thumbnail thumbnail thumbnail-size start end)))))

(defun org-link-colorize--preview-epub (path start end)
  ...
  (if (string-match "\\(.*?\\)\\(?:::\\(.*\\)\\)?\\'" path)
      (let* ((file-path (match-string 1 path))
             ...
             (thumbnail-size (or org-link-colorize-epub-preview-size 500)))
        (org-link-colorize--ensure-thumbnails-dir thumbnails-dir)
        ;; DEBUG:
        ;; (message epub-file)
        (unless (file-exists-p thumbnail)
          (cl-case system-type
            ('gnu/linux                 ; for Linux "gnome-epub-thumbnailer"
             (start-process
              "org-link-colorize--epub-preview"
              " *org-link-colorize epub-preview*"
              org-link-colorize-epub-preview
              epub-file thumbnail
              ;; (if org-link-colorize-epub-preview-size
              ;;     "--size")
              ;; (if org-link-colorize-epub-preview-size
              ;;     (number-to-string thumbnail-size))
              )
             (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
               (message "[org-link-colorize] epub create thumbnail for\n %s \nfailed." thumbnail)))
            ('darwin                    ; for macOS "epub-thumbnailer" command
             ;; DEBUG
             ;; (message epub-file)
             ;; (message thumbnail)
             ;; (message (number-to-string org-link-colorize-epub-preview-size))
             (make-process
              :name "org-link-colorize--epub-preview"
              :command (list org-link-colorize-epub-preview
                             epub-file
                             thumbnail
                             (number-to-string thumbnail-size))
              :buffer " *org-link-colorize epub-preview*"
              :sentinel (lambda (proc event)
                          (message (format "> proc: %s\n> event: %s" proc event))
                          (when (and org-link-colorize-enable-debug-p (string= event "finished\n"))
                            (message "org-link-colorize epub preview Process DONE!")
                            (kill-buffer (process-buffer proc))
                            ;; (kill-process proc)
                            ))
              :stdout " *org-link-colorize epub-preview*"
              :stderr " *org-link-colorize epub-preview*")
             (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
               (message "[org-link-colorize] epub create thumbnail for\n %s \nfailed." thumbnail)))
            (t (user-error "This system platform currently not supported by org-link-colorize.\n Please contribute code to support"))))
        (org-link-colorize--add-overlay-marker start end)
        (org-link-colorize--add-keymap start end)
        (org-link-colorize--display-thumbnail thumbnail thumbnail-size start end))))

(defun org-link-colorize--preview-text-file (file lines)
...
;;; test
;; (org-link-colorize--preview-text-file
;;  (expand-file-name "~/Code/Emacs/org-link-colorize/org-link-colorize.el")
;;  3)

(defun org-link-colorize--preview-text (path start end &optional lines)
  ...
  (let* ((text-file (expand-file-name (org-link-unescape path)))
         ...
         (preview-content (org-link-colorize--preview-text-file text-file preview-lines)))
    (org-link-colorize--add-overlay-marker (1+ end) (+ end 2))
    (org-link-colorize--add-keymap (1+ end) (+ end 2))
...
(defun org-link-colorize--preview-video (path start end)
  ...
  (let* ((video-file (expand-file-name (org-link-unescape path)))
         ...
         (thumbnail-size (or org-link-colorize-video-preview-size 512)))
    (org-link-colorize--ensure-thumbnails-dir thumbnails-dir)
    (unless (file-exists-p thumbnail)
      (cond
       ...
       ((and (eq system-type 'darwin) (executable-find "qlmanage")
        ...
        (start-process
         "org-link-colorize--video-preview"
         " *org-link-colorize video-preview*"
         "qlmanage"
         "-x"
         "-t"
        ...
        (let ((original-thumbnail-file (concat thumbnails-dir (file-name-nondirectory video-file) ".png")))
          (if (and (not org-link-colorize-enable-debug-p) (file-exists-p original-thumbnail-file))
              (rename-file original-thumbnail-file thumbnail)
            (message "[org-link-colorize] qlmanage create thumbnail for\n %s \nfailed." original-thumbnail-file))))
       ...
       ((executable-find "ffmpegthumbnailer")
        (start-process
         ...
         "-o" thumbnail)
        (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
          (message "[org-link-colorize] 'ffmpegthumbnailer' create thumbnail for\n %s \nfailed." thumbnail)))
       ;; use `ffmpeg'
       ...
       ((executable-find "ffmpeg")
        (start-process
         ...
         thumbnail)
        (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
          (message "[org-link-colorize] 'ffmpeg' create thumbnail for\n %s \nfailed." thumbnail)))))
    (org-link-colorize--add-overlay-marker start end)
    (org-link-colorize--add-keymap start end)
    (org-link-colorize--display-thumbnail thumbnail thumbnail-size start end)))

(defun org-link-colorize--preview-audio (path start end)
  ...
  (let* ((audio-file (expand-file-name (org-link-unescape path)))
         ...
         (thumbnail-size (or org-link-colorize-audio-preview-size 200)))
    (org-link-colorize--ensure-thumbnails-dir thumbnails-dir)
    (unless (file-exists-p thumbnail)
      ;; DEBUG:
      ...
      (cond
       ((and (eq system-type 'darwin) (executable-find "qlmanage"))
        (start-process
         "org-link-colorize--audio-preview"
         " *org-link-colorize audio preview*"
         "qlmanage"
         "-x"
         "-t"
       ...
       ((and (eq system-type 'gnu/linux) (executable-find "audiowaveform"))
        (start-process
         "org-link-colorize--audio-preview"
         " *org-link-colorize audio preview*" ; DEBUG: check out output buffer
         "audiowaveform"
         "-i" audio-file
         "-o" thumbnail)
        (when (and org-link-colorize-enable-debug-p (not (file-exists-p thumbnail)))
          (message "[org-link-colorize] 'audiowaveform' create thumbnail for\n %s \nfailed." thumbnail)))))
    (org-link-colorize--add-overlay-marker start end)
    (org-link-colorize--add-keymap start end)
    (org-link-colorize--display-thumbnail thumbnail thumbnail-size start end)))

(defun org-link-colorize--return-icon (type path extension &optional link-element)
  ...
  (pcase type
     ...
     (cond
      ...
      ((file-directory-p path)          ; directory
       (all-the-icons-icon-for-dir
        "path"
        :face (org-link-colorize--warning path)
        :v-adjust 0))
      ;; MindMap files
    ...
    ;; ("fuzzy"
    ;;  ;; DEBUG
    ;;  (message "[org-link-colorize] link-element: %s" link-element)
...
(defun org-link-colorize--display-icon (start end description icon)
...
(defun org-link-colorize--display-not-exist (start end description icon)
  ...
  (when (eq (car (org-link-colorize--get-element start)) 'link)
    (save-match-data
      (let* ((link-element (org-link-colorize--get-element start))
             ...
             (icon (org-link-colorize--return-icon type path extension link-element))
             ;; DEBUG:
        ...
        (cond
         ...
         ((and (equal type "file")
               (member extension org-link-colorize-video-preview-list)
               org-link-colorize-video-preview)
          (org-link-colorize--preview-video path start end))
         ;; audio wave form image preview
         ;; [[file:/path/to/audio.mp3]]
         ((and (equal type "file")
               (member extension org-link-colorize-audio-preview-list)
               org-link-colorize-audio-preview)
          (org-link-colorize--preview-audio path start end))
         ;; PDF file preview
         ;; [[file:/path/to/filename.pdf]]
         ;; [[pdf:/path/to/filename.pdf::15]]
         ;; [[pdfview:/path/to/filename.pdf::15]]
         ((and org-link-colorize-pdf-preview
               (or (and (equal type "file") (string= extension "pdf"))
                   (equal type "pdf")
                   (equal type "pdfview")
                   (equal type "docview")
                   (equal type "eaf")))
          (org-link-colorize--preview-pdf
           (if (equal type "eaf")
               (replace-regexp-in-string "pdf::" "" path)
             path)
           start end))
         ;; EPUB file cover preview
         ((and org-link-colorize-epub-preview
               (and (equal type "file") (string= extension "epub")))
          (org-link-colorize--preview-epub path start end))
         ;; text content preview
         ((and (equal type "file")
               (member extension org-link-colorize-text-preview-list)
               org-link-colorize-text-preview)
          (org-link-colorize--preview-text path start end))
         ;; file does not exist
         ((and (equal type "file") (not (file-exists-p path)))
          ;; DEBUG (message path)
          (org-link-colorize--add-overlay-marker start end)
          (org-link-colorize--display-not-exist start end description icon))
         ;; general icons
         (t
          ;; DEBUG:
          ...
          (org-link-colorize--add-overlay-marker start end)
          (org-link-colorize--add-keymap start end)
          (org-link-colorize--display-icon start end description icon)))))))
...
(defun org-link-colorize--add-more-icons-support ()
...
(defvar org-link-colorize-keymap (make-sparse-keymap))

(defun org-link-colorize--add-keymap (start end)
  "Add keymap on link text-property. between START and END."
  (put-text-property start end 'keymap org-link-colorize-keymap))

(define-key org-link-colorize-keymap (kbd "RET") 'org-open-at-point)
(define-key org-link-colorize-keymap [mouse-1] 'org-open-at-point)
(define-key org-link-colorize-keymap (kbd "<mouse-1>") 'org-open-at-point)
...
(defun org-link-colorize-enable ()
  ...
  (when (display-graphic-p)
    (org-link-colorize--add-more-icons-support)
